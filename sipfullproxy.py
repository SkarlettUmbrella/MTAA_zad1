...
rx_invalid = re.compile(r"99999999999999")
rx_invalid2 = re.compile(r"9999999999999999999")
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request (ty kokos...)")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Temporarily Unavailable (ty kokos...)")
        else:
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            if origin in registrar:
                ...
                text = "\r\n".join(data)
                if (data[0].find("Ok") != -1 and data[5].find("INVITE")):
                    logging.info("CALL ACCEPTED: %s %s" % (data[2].split(";")[0], data[3].split(";")[0]))
                elif (data[0].find("Ok") != -1 and data[5].find("BYE")):
                    logging.info("CALL ENDED: %s" % (data[2].split(";")[0]))
                text = text.replace("Ok", "PEKNE", 1)
                socket.sendto(text.encode("utf-8"), claddr)
                showtime()
                # logging.info("<<< %s" % data[0])
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri) or rx_code.search(request_uri):
            showtime()
            if (request_uri.split(" ")[0].find("REGISTER") == 0):
                logging.info("USER REGISTERED: %s", request_uri.split(" ")[1])
            elif (request_uri.split(" ")[0].find("INVITE") == 0):
                logging.info("INVITE SENT: %s", request_uri.split(" ")[1])
            elif (request_uri.split(" ")[0].find("ACK") == 0):
                logging.info("USER ACKNOWLEDGED: %s", request_uri.split(" ")[1].split(";")[0])
            elif (request_uri.split(" ")[0].find("BYE") == 0):
                logging.info("USER ENDED CALL: %s", request_uri.split(" ")[1])
            elif (request_uri.split(" ")[2].find("Trying") == 0):
                logging.info("Trying to call")
            elif (request_uri.split(" ")[2].find("Ringing") == 0):
                logging.info("Ringing to other client")
            elif (request_uri.split(" ")[2].find("Ok") == 0):
                logging.info("Replying with acknowledgement")
